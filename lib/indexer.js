...
module.exports = class Indexer {
    ...
    _check_hit(token_arr,search_key_length) {
        ...
        for(var rs in result_map){
            if(result_map[rs]>=search_key_length){
                console.log("==>"+result_map[rs]+","+rs);
                result_ids.push(rs);
            }
        }
    ...
    prefixes_to_tokens(prefixes) {
        var tokens = new Set();

        for (var p of prefixes) {
            for (var token of this.trie.keys_with_prefix(p)) {
                tokens.add(token);
            }
        }
    ...
    search_prefix(prefix) {
        ...
        var tokens = this.prefixes_to_tokens(prefixes);

        var result_items = this._check_hit(tokens,prefixes.length);
